...
import { DEFAULT_MASCOT, MASCOT_ANIMATIONS } from '../../config/mascot';
...
export const MascotCharacter: React.FC<MascotCharacterProps> = ({
  ...
  const currentExpression =
    mascot.expressions.find((e: any) => e.id === expression) || mascot.expressions[0];
  ...
  const startAnimation = () => {
    const animation = MASCOT_ANIMATIONS[currentExpression.animation];
  ...
  if (onPress) {
    return <View onStartShouldSetResponder={() => true}>{content}</View>;
  }
