...
const FeedbackScreen: React.FC = () => {
  ...
  const renderTypeOption = (type: 'bug' | 'feature' | 'other', label: string, icon: string) => {
    ...
    return (
      <TouchableOpacity
        style={[
          styles.typeOption,
          { backgroundColor: isDark ? '#1E1E1E' : '#FFFFFF', borderColor: isDark ? '#333' : '#FFF0F3' },
          isSelected && {
  ...
  return (
    ...
    >
      <View style={[styles.header, { paddingTop: insets.top, backgroundColor: isDark ? '#121212' : '#FAFAFA', borderBottomColor: isDark ? '#333' : '#F0F0F0' }]}>
        <TouchableOpacity
        ...
        <Text style={[styles.headerTitle, { color: isDark ? '#FFF' : HEALING_COLORS.gray[800] }]}>帮助与反馈</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false} indicatorStyle={isDark ? 'white' : 'black'}>
        <View style={styles.greetingSection}>
          ...
          <Text style={[styles.greetingTitle, { color: isDark ? '#FFF' : HEALING_COLORS.gray[800] }]}>Hi，遇到什么问题了吗？</Text>
          <Text style={[styles.greetingSub, { color: isDark ? '#9CA3AF' : HEALING_COLORS.gray[500] }]}>无论是报错还是新想法，都欢迎告诉毛球～</Text>
        </View>

        <View style={styles.formSection}>
          <Text style={[styles.label, { color: isDark ? '#E5E7EB' : HEALING_COLORS.gray[700] }]}>反馈类型</Text>
          ...
          <Text style={[styles.label, { marginTop: 24, color: isDark ? '#E5E7EB' : HEALING_COLORS.gray[700] }]}>想对毛球说的话</Text>
          <TextInput
            style={[styles.textArea, { borderRadius: themeStyle.borderRadius, backgroundColor: isDark ? '#1E1E1E' : '#FFFFFF', borderColor: isDark ? '#333' : '#FFF0F3', color: isDark ? '#FFF' : HEALING_COLORS.gray[800] }]}
            ...
            maxLength={500}
          />
          <Text style={[styles.wordCount, { color: isDark ? '#6B7280' : HEALING_COLORS.gray[400] }]}>{content.length}/500</Text>

          <Text style={[styles.label, { marginTop: 12, color: isDark ? '#E5E7EB' : HEALING_COLORS.gray[700] }]}>图片/视频（最多3个）</Text>
          <View style={styles.mediaContainer}>
            <MediaSelector media={media} onMediaChange={setMedia} maxCount={3} hideHeader />
          </View>

          <Text style={[styles.label, { marginTop: 12, color: isDark ? '#E5E7EB' : HEALING_COLORS.gray[700] }]}>联系方式（选填）</Text>
          <TextInput
            style={[styles.input, { borderRadius: themeStyle.borderRadius, backgroundColor: isDark ? '#1E1E1E' : '#FFFFFF', borderColor: isDark ? '#333' : '#FFF0F3', color: isDark ? '#FFF' : HEALING_COLORS.gray[800] }]}
          ...
          <TouchableOpacity
            style={[
              styles.submitButton,
              { borderRadius: themeStyle.borderRadius, backgroundColor: isDark ? HEALING_COLORS.pink[600] : HEALING_COLORS.pink[500], shadowColor: isDark ? '#000' : HEALING_COLORS.pink[500] },
